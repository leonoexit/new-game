...
assert.equal(GAME.farmFieldCapacity, 2);
...
assert.equal(game.cards.filter((item) => item.meta.isLand).length, 2, "the Farm begins at its finite two-field capacity");
...
oneCard(game, "sickle");
assert.equal(game.cards.filter((item) => item.meta.isLand).length, 2, "crop cycles never create or destroy Land cards");
...
assert.equal(boundaryGame.cards.filter((item) => item.meta.isLand).length, 2, "weekly journals preserve Land identity");
...
assert.match(app, /card-description/);
...
assert.match(app, /id="reset">Reset farm/);
...
assert.match(styles, /\.card-description/);
assert.doesNotMatch(styles, /:hover \.card-description|:focus-visible \.card-description|Pull to detach/);
